...
use tokio::fs::{read_dir, metadata, read_to_string};

struct DiskIO;

impl DiskIO
{
/// Recursively reads all files in a directory and its subdirectories, processing them into `InputPage` objects.
///
/// # Arguments
///
/// * `dir_path` - A string slice that holds the path to the directory to be read.
///
/// # Returns
///
/// A pinned future that resolves to a `Result` containing a vector of `InputPage` objects if successful, or an error if the operation fails.
///
/// # Behavior
///
/// - Traverses the directory and its subdirectories.
/// - For each file, it calls `process_file` to process the file and append its content to the output vector.
/// - If a subdirectory is encountered, it recursively processes the subdirectory.
pub fn read_directory_recursively(dir_path: &str) -> Pin<Box<dyn Future<Output = Result<Vec<InputPage>>> + '_>> {
  Box::pin(async move {
    let mut output = Vec::new();

    let mut entries = read_dir(dir_path).await.expect("Unable to read directory");

    while let Some(entry) = entries.next_entry().await.expect("Unable to read entry") {
      let path = entry.path();
      if path.is_dir() {
...
/// Reads all files in a given directory (non-recursively) and processes them into `InputPage` objects.
///
/// # Arguments
///
/// * `dir_path` - A string slice that holds the path to the directory to be read.
...
/// # Returns
///
/// A `Result` containing a vector of `InputPage` objects if successful, or an error if the operation fails.
///
/// # Behavior
///
/// - Iterates through all entries in the specified directory.
/// - For each file, it calls `process_file` to process the file and append its content to the output vector.
/// - Only processes files in the given directory, does not traverse subdirectories.
...
/// Processes a single file and appends its content to the provided output vector.
/// 
/// # Arguments
/// 
/// * `output` - A mutable reference to a vector of `InputPage` objects where the processed file's content will be stored.
/// * `path` - The path to the file to be processed.
/// 
/// # Behavior
/// 
/// - If the file is a PDF, it extracts its pages using `read_pdf_file` and appends them to the output.
/// - If the file is a TXT file, it reads its content using `read_txt_file` and appends it to the output.
async fn process_file(output: &mut Vec<InputPage>, path: PathBuf) {
  if path.is_file() {
    let file_path = path.to_str().unwrap();
    if file_path.ends_with(".pdf") {
      let pages = DiskIO::read_pdf_file(file_path).await.expect("Unable to read PDF file");
      output.extend(pages);
    } else if file_path.ends_with(".txt") {
      let pages = DiskIO::read_txt_file(file_path).await.expect("Unable to read TXT file");
      output.extend(pages);
    }
  }
}
...
/// # Arguments
///
/// * `file_path` - A string slice that holds the path to the text file to be read.
///
/// # Returns
///
/// A `Result` containing a vector of `InputPage` objects if successful, or an error if the operation fails.
...
mod tests
{
  ...
  async fn io_should_read_directory_recursively()
  {
    ...
    // Checks if the titles of multiple directories are present. And checks for file names with spaces in the title.
    let art = pages.iter().any(|page| page.file_name.contains("visualelementsandprinciplesofcomposition.pdf"));
    let electronics = pages.iter().any(|page| page.file_name.contains("Lecture16_ppt.pdf"));
    let biology = pages.iter().any(|page| page.file_name.contains("Themes and Concepts of Biology.pdf"));
  ...
  async fn io_should_read_directory_flat()
  {
    ...
    // Act
    let pages = DiskIO::read_directory_files(dir_path).await.expect("Unable to read directory");
   
    let title1 = pages.iter().any(|page| page.file_name.contains("Lecture10_ppt.pdf"));
    let title2 = pages.iter().any(|page| page.file_name.contains("Lecture16_ppt.pdf"));
    // Assert
